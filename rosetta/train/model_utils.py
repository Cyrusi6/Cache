...
def k_nearest_sources(
    num_target_layers: int, num_source_layers: int, k: int
) -> Dict[int, List[int]]:
    """
    ...
    if num_target_layers <= 1:
        ...
        target_positions = [
            i / (num_target_layers - 1) for i in range(num_target_layers)
        ]
    if num_source_layers <= 1:
        source_positions = [0.0]
    else:
        source_positions = [
            j / (num_source_layers - 1) for j in range(num_source_layers)
        ]

    mapping: Dict[int, List[int]] = {}
    for t_idx, t_pos in enumerate(target_positions):
        sorted_src = sorted(
            range(num_source_layers), key=lambda j: abs(source_positions[j] - t_pos)
        )
...
def last_aligned_sources(
    num_target_layers: int, num_source_layers: int, k: int = 1
) -> Dict[int, List[int]]:
    """
...
def setup_models(
    ...
    if projector_params.get("alignment_confidence_layer_scale_mode", "none") != "none":
        projector_params.setdefault("alignment_confidence_layer_idx", 0)
        projector_params.setdefault("alignment_confidence_num_layers", 1)
    ...
    num_layers_to_map = min(
        base_model.config.num_hidden_layers, teacher_model.config.num_hidden_layers
    )
