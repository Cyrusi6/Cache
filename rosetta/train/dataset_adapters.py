...
def create_text_length_filter(
    ...
    def _text_length_filter_batch(batch):
        ...
        samples = [
            {key: values[i] for key, values in batch.items()} for i in range(batch_size)
        ]
        try:
            texts = [text_extractor(sample) for sample in samples]
            if use_tokens:
                if hasattr(tokenizer, "apply_chat_template") and any(
                    isinstance(t, list) for t in texts
                ):
                    rendered = []
                    for t in texts:
                    ...
                    tokenized = tokenizer(
                        [str(t) for t in texts], add_special_tokens=False
                    )
...
def create_field_value_filter(
    target_value: Any, field_name: str, comparison: str = "equal"
):
    """
...
def create_modulo_filter(
    mod_base: int, exclude_values: Union[int, List[int]], field_name: str = "_id"
):
    """
...
def create_conversation_length_filter(
    min_messages: int, text_field: str = "conversations"
):
    """
...
def extract_chat_text(
    ...
    assistant_message = (
        answers[0] if answers and len(answers) > 0 else "No answer provided"
    )
...
def extract_conversation_text(
    sample: Dict[str, Any], text_field: str = "conversations"
) -> str:
...
def extract_first_assistant_message(
    sample: Dict[str, Any], text_field: str = "conversations"
) -> str:
...
def extract_openhermes_messages(
    sample: Dict[str, Any], text_field: str = "conversations"
) -> List[Dict[str, str]]:
    ...
    for msg in conversation:
        ...
        if role in ("human", "user"):
            messages.append(
                {"role": "user", "content": str(msg.get("value", "")).strip()}
            )
...
def extract_chat_pair_text(
    sample: Dict[str, Any], user_field: str = "inputs", assistant_field: str = "targets"
) -> List[Dict[str, str]]:
...
def extract_mmmlu_chat_messages(sample: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    user_prompt = template.replace("{{choices}}", choices_text).replace(
...
def apply_batch_filters(
    ...
    if combine_filters and len(filters) > 1:
        ...
        if original_len != final_len:
            print(
                f"Applied combined batch filtering: {original_len} -> {final_len} samples"
            )
        ...
        for i, (filter_func, desc) in enumerate(
            zip(filters, filter_descriptions or [""] * len(filters))
        ):
        ...
        if original_len != final_len:
            print(
                f"Applied sequential batch filtering: {original_len} -> {final_len} samples"
            )
...
def generate_kv_cache_index(instruction_length: int, full_length: int) -> torch.tensor:
    ...
    instruction_index = torch.tensor([1, 0], dtype=torch.long).repeat(
        instruction_length - 1, 1
    )
    label_index = torch.tensor([-1, 0], dtype=torch.long).repeat(
        full_length - instruction_length + 1, 1
    )

    kv_cache_index = torch.cat(
        [instruction_index, label_index], dim=0
    )  # shape: (seq_len, 2)
...
class LongBenchChatDataset(Dataset):
    ...
    def __init__(
        ...
        target_datasets = (
            longbench_e_datasets if use_longbench_e else longbench_datasets
        )
        ...
        if dataset_name:
            if dataset_name not in target_datasets:
                raise ValueError(
                    f"Dataset {dataset_name} not found in LongBench{' -E' if use_longbench_e else ''}"
                )
        ...
        if filter_mod4:
            ...
            print(
                f"Filtered by _id mod4 != 1: {original_len} -> {len(self.dataset)} samples"
            )
        ...
        print(
            f"Loaded total {len(self.dataset)} samples from LongBench{' -E' if use_longbench_e else ''}"
        )
    ...
    def _format_longbench_example(
        self, example: Dict[str, Any], tokenizer: AutoTokenizer
    ) -> str:
        ...
        subject = (
            re.sub(r"_e$", "", current_subject)
            if self.use_longbench_e
...
class MMLUChatDataset(Dataset):
        ...
        if max_word_count is not None:
            ...
            filters = [
                create_text_length_filter(
                    max_word_count, extractor, self._mmlu_tokenizer, use_tokens=True
                )
            ]
            filter_descriptions = [
                f"Token count filter (full chat): max {max_word_count}"
            ]
            self.dataset, _ = apply_batch_filters(
                self.dataset, filters, filter_descriptions
            )
    ...
    def _build_chat_messages(self, sample: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if isinstance(ans_idx, str) and ans_idx.isdigit():
            ans_idx = int(ans_idx)
        ans_label = (
            choice_labels[ans_idx]
            if 0 <= int(ans_idx) < len(choice_labels)
            else chr(65 + int(ans_idx))
        )
...
class LLMGeneratedChatDataset(Dataset):
    ...
    def __init__(
        ...
        split: str = "train",
        num_samples: Optional[int] = None,
        data_path: str = "./teacher_datasets/output/dataset_finished",
        max_word_count: Optional[int] = None,
    ):
        """
        Initialize the dataset
        ...
        if max_word_count is not None:
            ...
            print(
                f"Filtered by max_word_count={max_word_count}: {original_len} -> {len(self.dataset)} samples"
            )
...
class OpenBookChatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        user_prompt = f"Question: {sample['question_stem']}\n\n" f"Choices:\n"
...
class OpenHermesChatDataset(Dataset):
    ...
    def __init__(
        ...
        if min_conversation_turns > 0:
            filters.append(
                create_conversation_length_filter(
                    min_conversation_turns - 1, "conversations"
                )
            )
            filter_descriptions.append(
                f"Conversation length filter: min {min_conversation_turns} messages (multi-turn only)"
            )
        ...
        if max_word_count is not None:
            ...
            filters.append(
                create_text_length_filter(
                    max_word_count, extractor, tokenizer, use_tokens=True
                )
            )
        ...
        if filters:
            self.dataset, _ = apply_batch_filters(
                self.dataset, filters, filter_descriptions, num_proc=8
            )
...
class ChatDataset(Dataset):
    ...
    def __getitem__(self, idx) -> Dict[str, Any]:
        ...
        labels = [-100] * len(instruction_tokens) + full_tokens[
        ...
        kv_cache_index = generate_kv_cache_index(
            len(instruction_tokens), len(full_tokens)
        )
...
class AlignedChatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        instr_end = min(instr_end, len(slm_ids))
    ...
    def _load_candidate_replay_cache(
        ...
        with open(cache_path, "r") as f:
            for line_num, line in enumerate(f, start=1):
                ...
                if "idx" not in item:
                    raise ValueError(
                        f"Candidate replay cache row {line_num} missing 'idx'."
                    )
                result[int(item["idx"])] = item
        return result

    def _candidate_replay_entry(self, idx: int) -> Optional[Dict[str, torch.Tensor]]:
        item = self.candidate_replay_cache.get(int(idx))
        if item is None:
            return None
        if not bool(item.get("cached", True)):
            return None
        if "target" not in item or "utility" not in item:
            return None
        target = torch.tensor(item.get("target", []), dtype=torch.float)
        utility = torch.tensor(item.get("utility", []), dtype=torch.float)
        if target.numel() == 0 or utility.numel() == 0:
            return None
        if "utility_valid" in item:
            utility_valid = torch.tensor(item["utility_valid"], dtype=torch.bool)
        else:
        ...
        return {
            ...
            "cache_hit": torch.tensor(True, dtype=torch.bool),
        }

    @staticmethod
    def _renormalize_soft_alignment(
        source_indices: torch.Tensor,
        source_weights: torch.Tensor,
        source_length: int,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Drop indices outside the retained source sequence and renormalize rows."""
        if source_indices.numel() == 0:
            return source_indices, source_weights

        invalid = (source_indices < 0) | (source_indices >= source_length)
        source_indices = source_indices.clone()
        source_weights = source_weights.clone()
        source_indices[invalid] = -1
        source_weights[invalid] = 0.0
        row_sums = source_weights.sum(dim=-1, keepdim=True)
        source_weights = torch.where(
            row_sums > 0,
            source_weights / row_sums.clamp_min(1e-12),
            source_weights,
        )
        return source_indices, source_weights
    ...
    def _getitem_soft(self, messages: List[Dict[str, str]], idx: int) -> Dict[str, Any]:
        ...
        message_mask = torch.tensor(details["message_mask"], dtype=torch.bool)
        source_indices = torch.tensor(
            details["soft_alignment"]["source_indices"], dtype=torch.long
        )
        source_weights = torch.tensor(
            details["soft_alignment"]["source_weights"], dtype=torch.float
        )
        source_confidence = torch.tensor(
            details["soft_alignment"].get(
                "source_confidence",
                [1.0] * len(details["soft_alignment"]["source_indices"]),
            ),
            dtype=torch.float,
        )
        ...
        if candidate_replay is not None:
            item["candidate_replay"] = candidate_replay
        return item
...
class RosettaDataCollator:
    ...
    def __init__(
        ...
        if self.do_alignment:
            assert (
                self.aligner is not None
            ), "Aligner must be provided if do_alignment is True"
    ...
    def _split_into_sections(
        self, normalized_feature: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(len(change_points) - 1):
            ...
            section = {
                "input_ids": [
                    ids[start:end] for ids in normalized_feature["input_ids"]
                ],
                "attention_mask": [
                    mask[start:end] for mask in normalized_feature["attention_mask"]
                ],
    ...
    def _collect_section_data(
        self, all_sections: List[List[Dict[str, Any]]], sec_idx: int, num_models: int
    ) -> Dict[str, List]:
        ...
        for sample_sections in all_sections:
            ...
            if sec_idx < len(sample_sections):
                ...
                for model_idx in range(num_models):
                    section_data["input_ids_per_model"][model_idx].append(
                        sec["input_ids"][model_idx]
                    )
                    section_data["attention_mask_per_model"][model_idx].append(
                        sec["attention_mask"][model_idx]
                    )
                ...
                section_data["kv_cache_index"].append(
    ...
    def _pad_single_section(
        self, section_data: Dict[str, List], num_models: int
    ) -> Dict[str, Any]:
        ...
        for model_idx in range(num_models):
            pad_token_id = (
                self.slm_pad_token_id if model_idx == 0 else self.llm_pad_token_id
            )
    ...
    def _finalize_output(
        self, padded_output: Dict[str, List], num_models: int, batch_size: int
    ) -> Dict[str, Any]:
        ...
        if num_models == 1:
            ...
            final_output["input_ids"] = torch.cat(
                padded_output["input_ids_per_model"][0], dim=1
            )
            final_output["attention_mask"] = torch.cat(
                padded_output["attention_mask_per_model"][0], dim=1
            )
        else:
        ...
        final_output["kv_cache_index"] = padded_output[
            "kv_cache_index"
        ]  # Keep as list of sections
    ...
    def _apply_length_constraints(self, output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(output["input_ids"], list):
            output["input_ids"] = [
                ids[:, : self.max_length] for ids in output["input_ids"]
            ]
            output["attention_mask"] = [
                mask[:, : self.max_length] for mask in output["attention_mask"]
            ]
        else:
    ...
    def _truncate_kv_cache_sections(
        self, kv_cache_sections: List[torch.Tensor], max_length: int
    ) -> List[torch.Tensor]:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if "soft_alignment" in features[0]:
            return self._collate_soft_alignment(features)
    ...
    def _collate_soft_alignment(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Collate soft-aligned samples with independent source sequence length."""
        target_sections: List[List[Dict[str, torch.Tensor]]] = []
        source_input_ids = []
        source_attention_masks = []
        candidate_replay_entries = []

        for feature in features:
            slm_ids = torch.tensor(feature["input_ids"][0], dtype=torch.long)
            llm_ids = torch.tensor(feature["input_ids"][1], dtype=torch.long)
            slm_attention = (~feature["model_padding_mask"][0]).float()
            llm_attention = (~feature["model_padding_mask"][1]).float()
            labels = torch.tensor(feature["labels"], dtype=torch.long)
            kv_idx = feature["kv_cache_index"]
            source_indices = feature["soft_alignment"]["source_indices"]
            source_weights = feature["soft_alignment"]["source_weights"]
            source_confidence = feature["soft_alignment"].get(
                "source_confidence",
            ...
            source_attention_masks.append(llm_attention)
            candidate_replay_entries.append(feature.get("candidate_replay"))
            ...
            sections: List[Dict[str, torch.Tensor]] = []
            for sec_idx in range(len(change_points) - 1):
                start, end = change_points[sec_idx], change_points[sec_idx + 1]
                sections.append(
                    {
                        "input_ids": slm_ids[start:end],
                        "attention_mask": slm_attention[start:end],
                        "labels": labels[start:end],
                        "kv_cache_index": kv_idx[start:end],
        ...
        max_sections = max(len(sections) for sections in target_sections)
        padded_target_ids = []
        padded_target_attention = []
        padded_labels = []
        padded_position_ids = []
        padded_kv_cache = []
        padded_soft_alignment = []
        ...
        for sec_idx in range(max_sections):
            ...
            for sections in target_sections:
                if sec_idx < len(sections):
                    sec = sections[sec_idx]
                    sec_inputs.append(sec["input_ids"])
                    sec_attention.append(sec["attention_mask"])
                    sec_labels.append(sec["labels"])
                    sec_positions.append(sec["position_ids"])
                    sec_kv.append(sec["kv_cache_index"])
                    ...
                    if top_k is None:
                        top_k = target_sections[0][0]["source_indices"].shape[-1]
                    sec_inputs.append(torch.tensor([], dtype=torch.long))
                    sec_attention.append(torch.tensor([], dtype=torch.float))
                    sec_labels.append(torch.tensor([], dtype=torch.long))
                    sec_positions.append(torch.tensor([], dtype=torch.long))
                    sec_kv.append(torch.empty((0, 2), dtype=torch.long))
            ...
            padded_target_ids.append(
                torch.nn.utils.rnn.pad_sequence(
                    sec_inputs, batch_first=True, padding_value=self.slm_pad_token_id
                )
            )
            padded_target_attention.append(
                torch.nn.utils.rnn.pad_sequence(
                    sec_attention, batch_first=True, padding_value=0
                )
            )
            padded_labels.append(
                torch.nn.utils.rnn.pad_sequence(
                    sec_labels, batch_first=True, padding_value=-100
                )
            )
            padded_position_ids.append(
                torch.nn.utils.rnn.pad_sequence(
                    sec_positions, batch_first=True, padding_value=0
                )
            )
            ...
            padded_soft_alignment.append(
                {
                    "source_indices": torch.nn.utils.rnn.pad_sequence(
                        sec_indices, batch_first=True, padding_value=-1
                    ),
                    "source_weights": torch.nn.utils.rnn.pad_sequence(
                        sec_weights, batch_first=True, padding_value=0.0
                    ),
        ...
        target_input_ids = torch.cat(padded_target_ids, dim=1)
        target_attention_mask = torch.cat(padded_target_attention, dim=1)
        labels = torch.cat(padded_labels, dim=1)
        position_ids = torch.cat(padded_position_ids, dim=1)

        source_ids = torch.nn.utils.rnn.pad_sequence(
            source_input_ids, batch_first=True, padding_value=self.llm_pad_token_id
        )
        source_attention = torch.nn.utils.rnn.pad_sequence(
            source_attention_masks, batch_first=True, padding_value=0
        )

        output = {
            "input_ids": [target_input_ids, source_ids],
            "attention_mask": [target_attention_mask, source_attention],
            "labels": labels,
            "position_ids": position_ids,
            "kv_cache_index": padded_kv_cache,
            "soft_alignment": padded_soft_alignment,
        }

        if any(entry is not None for entry in candidate_replay_entries):
            first_entry = next(
                entry for entry in candidate_replay_entries if entry is not None
            )
            replay_width = int(first_entry["target"].numel())
            replay_targets = []
            replay_utilities = []
            replay_utility_valid = []
            replay_cache_hit = []
            for entry in candidate_replay_entries:
                if entry is None:
                    replay_targets.append(torch.zeros(replay_width, dtype=torch.float))
                    replay_utilities.append(
                        torch.zeros(replay_width, dtype=torch.float)
                    )
                    replay_utility_valid.append(
                        torch.zeros(replay_width, dtype=torch.bool)
                    )
                    replay_cache_hit.append(torch.tensor(False, dtype=torch.bool))
                    continue
                target = entry["target"].to(dtype=torch.float)
                utility = entry["utility"].to(dtype=torch.float)
                utility_valid = entry["utility_valid"].to(dtype=torch.bool)
                if (
                    target.numel() != replay_width
                    or utility.numel() != replay_width
                    or utility_valid.numel() != replay_width
                ):
                    raise ValueError(
                        "All candidate replay cache entries in a batch must have "
                        "the same candidate width."
                    )
                replay_targets.append(target)
                replay_utilities.append(utility)
                replay_utility_valid.append(utility_valid)
                replay_cache_hit.append(entry["cache_hit"].to(dtype=torch.bool))
            output["candidate_replay"] = {
                "target": torch.stack(replay_targets, dim=0),
                "utility": torch.stack(replay_utilities, dim=0),
                "utility_valid": torch.stack(replay_utility_valid, dim=0),
                "cache_hit": torch.stack(replay_cache_hit, dim=0),
            }

        if self.max_length is not None and target_input_ids.size(1) > self.max_length:
            output["input_ids"][0] = output["input_ids"][0][:, : self.max_length]
            output["attention_mask"][0] = output["attention_mask"][0][
                :, : self.max_length
            ]
            output["labels"] = output["labels"][:, : self.max_length]
            output["position_ids"] = output["position_ids"][:, : self.max_length]
            ...
            output["soft_alignment"] = self._truncate_soft_alignment_sections(
    ...
    def _truncate_soft_alignment_sections(
        ...
        for section in soft_sections:
            ...
            if remaining_length <= 0:
                break
            if remaining_length >= section_length:
                truncated_sections.append(section)
                current_pos += section_length
            else:
                truncated_sections.append(
                    {
                        ...
                        "source_weights": section["source_weights"][
                            :, :remaining_length
                        ],
