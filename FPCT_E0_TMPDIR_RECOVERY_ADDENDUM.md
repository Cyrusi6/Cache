...
The recursive difference was exactly one field: W&B offline initialization installed `wandb.sdk.lib.import_hooks.ImportHookFinder` in `sys.meta_path`. The operative recovery therefore preinstalls that same W&B finder before the bootstrap computes its sealed pre-target fingerprint. It does so by registering and immediately unregistering a never-imported empty hook; the finder remains, and the later ordinary W&B initialization reuses it. The launcher does not change W&B mode, target code, training configuration, RNG, or model execution. The formal bootstrap target remains the immutable `SFT_train.py`.
