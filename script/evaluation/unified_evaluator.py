...
class UnifiedEvaluator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.dataset_name == "longbench":
            prompt_format_path = self.eval_config.get(
        ...
        if self.use_two_stage:
            ...
            if self.model_config["model_name"].lower() == "two_stage":
                ...
                self.rosetta_checkpoint_dir = self.model_config.get(
                    "rosetta_checkpoint_dir"
                )
                self.rosetta_subfolder = self.model_config.get(
    ...
    def _dump_bad_sample(
        ...
        try:
            ...
            dump_path = (
                dump_dir / f"bad_{self.dataset_name}_{subject}_{question_id}.json"
            )
            record = {
    ...
    def _format_mmlu_redux_example(
        self, example: Dict[str, Any], use_cot: bool, use_template: bool = True
    ) -> str:
    ...
    def _prepare_gpqa_item(self, example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        seed_int = int(hashlib.md5(seed_source.encode("utf-8")).hexdigest(), 16) % (
            2**32
        )
    ...
    def _format_math_problem_example(
        self, example: Dict[str, Any], use_cot: bool, use_template: bool = True
    ) -> str:
    ...
    def _format_openbookqa_example(
        self, example: Dict[str, Any], use_cot: bool, use_template: bool = True
    ) -> str:
    ...
    def parse_answer(self, example: Dict[str, Any]) -> Optional[str]:
        ...
        if self.dataset_name == "mmmlu":
            ...
            if isinstance(answer_key, str) and answer_key in [
                "A",
                "B",
                "C",
                "D",
    ...
    def create_segmented_kv_cache_index(
        ...
        if order_mode not in ["front", "back"]:
            raise ValueError(
                f"order_mode must be 'front' or 'back', got '{order_mode}'"
            )
        ...
        for i in range(1, len(complete_sequence)):
            if complete_sequence[i] != current_value:
                ...
                segment = (
                    torch.tensor(current_value, dtype=torch.long)
                    .repeat(segment_length, 1)
                    .unsqueeze(0)
                    .to(device)
                )
    ...
    def prepare_model_inputs(
        ...
        if use_aligner:
            alignment_strategy = self.model_config["rosetta_config"].get(
                "alignment_strategy", "prefix"
            )
            soft_alignment_top_k = int(
                self.model_config["rosetta_config"].get(
                    "soft_alignment_top_k",
                    self.eval_config.get("soft_alignment_top_k", 4),
                )
            )
            aligner = TokenAligner(
                ...
                soft_alignment_score_mode=self.model_config["rosetta_config"].get(
                    "soft_alignment_score_mode", "overlap"
                ),
                soft_alignment_boundary_bonus=self.model_config["rosetta_config"].get(
                    "soft_alignment_boundary_bonus", 0.0
                ),
                soft_alignment_boundary_tolerance=self.model_config[
                    "rosetta_config"
                ].get("soft_alignment_boundary_tolerance", 1),
                soft_alignment_min_weight=self.model_config["rosetta_config"].get(
                    "soft_alignment_min_weight", 0.0
                ),
                soft_alignment_confidence_mode=self.model_config["rosetta_config"].get(
                    "soft_alignment_confidence_mode", "none"
                ),
                soft_alignment_confidence_alpha=self.model_config["rosetta_config"].get(
                    "soft_alignment_confidence_alpha", 0.5
                ),
                soft_alignment_confidence_floor=self.model_config["rosetta_config"].get(
                ...
                soft_alignment_reweight_strength=self.model_config[
                    "rosetta_config"
                ].get("soft_alignment_reweight_strength", 1.0),
                soft_alignment_reweight_power=self.model_config["rosetta_config"].get(
                    "soft_alignment_reweight_power", 2.0
                ),
                soft_alignment_candidate_window=self.model_config["rosetta_config"].get(
            ...
            if alignment_strategy in {
                ...
                slm_attention_mask = torch.ones_like(slm_ids, dtype=torch.float)
                llm_attention_mask = torch.ones_like(llm_ids, dtype=torch.float)
                source_indices = torch.tensor(
                    details["soft_alignment"]["source_indices"],
                    dtype=torch.long,
                )
                source_weights = torch.tensor(
                    details["soft_alignment"]["source_weights"],
                    dtype=torch.float,
                )
                ...
                slm_ids = (
                    torch.tensor(details["slm_ids_padded"]).unsqueeze(0).to(device)
                )
                llm_ids = (
                    torch.tensor(details["llm_ids_padded"]).unsqueeze(0).to(device)
                )

                assert (
                    slm_ids.shape == llm_ids.shape
                ), f"SLM and LLM input lengths do not match: {slm_ids.shape} vs {llm_ids.shape}"
    ...
    def evaluate_subject(
    ...
    ) -> Tuple[Optional[np.ndarray], float, Optional[np.ndarray], List, List]:
        """
        ...
        if self.dataset_name == "longbench":
            ...
            if is_longbench_e:
                ...
                output_base_dir = (
                    self.output_dir
                    / "pred_e"
                    / self.model_config["model_name"].split("/")[-1]
                )
            else:
                # 普通学科使用pred目录
        ...
        for i in tqdm(
            ...
            try:
                ...
                if self.dataset_name != "longbench":
                    ...
                    id_hash = int(
                        hashlib.sha256(str(example["_id"]).encode("utf-8")).hexdigest(),
                        16,
                    )
                ...
                if model_type in ["two_stage", "two_stage_rosetta"]:
                    ...
                    if self.eval_config["answer_method"] == "logits":
                        ...
                        (outputs, bg_context), latency_ms = self._measure_latency_ms(
                            _two_stage_forward_call, device
                        )
                        ...
                        def _two_stage_call():
                            return model.generate(
                                question_without_options=question_text,
                                question_with_options=prompt_with_options,
                                communication_max_new_tokens=self.eval_config.get(
                                    "communication_max_new_tokens", 1024
                                ),
                                response_max_new_tokens=self.eval_config.get(
                                    "response_max_new_tokens", 1024
                                ),
                            )

                        content, latency_ms = self._measure_latency_ms(
                            _two_stage_call, device
                        )
                        ...
                        math_eval = None
                        if (
                            self.dataset_name in ["math-500", "gsm8k"]
                            and rule_evaluator is not None
                        ):
                            is_corr, extracted = rule_evaluator.rule_judge(
                                content, true_answer, finish_generation=True
                            )
                            math_eval = {
                        ...
                        input_length = len(
                            tokenizer.encode(
                                prompt_with_options, add_special_tokens=False
                            )
                        )
                        gen_length = len(
                    ...
                    if self.eval_config["answer_method"] == "logits":
                        ...
                        input_length, gen_length = None, None
                        cot_pred, cot_input_len, cot_gen_len, cot_text = (
                            None,
                            None,
                            None,
                            None,
                        ...
                        if isinstance(model, RosettaModel):
                            ...
                            if isinstance(prepared["inputs"]["input_ids"], list):
                                input_length = prepared["inputs"]["input_ids"][0].shape[
                                    1
                                ]
                            else:
                        ...
                        raise ValueError(
                            f"Unknown answer method: {self.eval_config['answer_method']}"
                        )
                ...
                if not printed_example:
                    try:
                        if model_type in ["two_stage", "two_stage_rosetta"]:
                            text = (
                                prompt  # Just show the formatted prompt for two-stage
                            )
                        else:
                        ...
                        if isinstance(text, (tuple, list)):
                            try:
                                ...
                                print(
                                    "[Input with chat template - SLM]:\n"
                                    + str(slm_text)
                                )
                                print(
                                    "[Input with chat template - LLM]:\n"
                                    + str(llm_text)
                                )
                            except Exception:
                            ...
                            print("[Input with chat template]:\n" + str(text))
                        if (
                            self.eval_config["answer_method"] == "generate"
                            and cot_text is not None
                        ):
                            print("\n[Generated output]:\n" + str(cot_text))
                ...
                cot_log_entry = {
                    ...
                    "question_id": i,
                    "true_answer": (
                        true_answer if self.dataset_name != "longbench" else None
                    ),
                    "pred": pred,
                    "is_correct": is_correct,
                    ...
                    "answer_latency_ms": (
                        float(latency_ms)
                        if "latency_ms" in locals() and latency_ms is not None
                        else None
                    ),
                }
                ...
                if self.dataset_name == "mmmlu":
                    ...
                    if "math_eval" in locals() and math_eval is not None:
                        cot_log_entry.update(
                            {
                                "extraction_method_used": math_eval.get(
                                    "extraction_method_used", ""
                                ),
                                "ground_truth_normalized": math_eval.get(
                                    "ground_truth_normalized", ""
                                ),
                                "extracted_normalized": math_eval.get(
        ...
        if total_count > 0 and self.dataset_name != "longbench":
            acc = np.mean(cors)
            print(
                f"{subject} accuracy: {acc*100:.2f}% (evaluated on {total_count} samples, skipped {skip_count})"
            )
        else:
            acc = 0
        ...
        return (
            np.array(cors) if cors else None,
            acc,
        ...
        if "two_stage_rosetta" == self.model_config["model_name"].lower():
            model = TwoStageRosetta(
                ...
                max_new_tokens=self.generation_config.get(
                    "max_new_tokens", self.eval_config.get("max_new_tokens", 1024)
                ),
            ...
            for subject, stats in subject_stats.items():
                if stats:
                    summary["subjects"][subject] = {
                        ...
                        "avg_length_ratio": np.mean([s["length_ratio"] for s in stats]),
                        "accuracy": (
                            np.mean([s["is_correct"] for s in stats])
                            if "is_correct" in stats[0]
                            else None
                        ),
                        "total_samples": len(stats),
                    }
