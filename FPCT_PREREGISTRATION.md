...
FPCT-1A 推荐的 `S>=2/3` 是 label-free support gate 的 strong/effective threshold，不回改这里的正式 matched-training structural eligibility `m>=2`。Gate-primary、formal performance primary 与 sensitivity strata 必须在所有 manifest 中分字段记录。
...
| FPCT-1A | Ambiguity support audit protocol locking | FPCT-0 GO | R0 | 定义、schema 与所有人工决策在自然 audit 前锁定 | threshold/support-floor 参数待人工批准 | FPCT-0 guard 失效、协议无法阻断结果驱动选择 |
| FPCT-1B | Label-free ambiguity support audit | FPCT-1A GO + 显式授权 | R1 CPU；禁止 forward | `LIMITED GO` 或 `CROSS-PAIR GO` | required cell/hash/schema 不完整 | 0 个 heterogeneous pair 通过 support floor |
...
| FPCT-2 | 隔离实现 `C_post`/`F` 与 manifest plumbing | FPCT-1B `LIMITED GO` 或 `CROSS-PAIR GO` + FPCT-1C GO + 人工授权 | R1 | 默认路径不变、operator 与 manifest 可选择 | 实现不完整但无反证 | 必须修改/污染 Phase 2A 路径或无法保持三臂容量 matched |
...
- FPCT-0 与 FPCT-1A 均不 commit、不 push，等待人工审查。
...
## 17. FPCT-1 prospective addendum：Ambiguity support gate

> Addendum date：2026-07-19
> 时间边界：在任何新的自然 ambiguity audit、逐样本 alignment 统计或 FPCT model result 之前

本 addendum 不改变 primary hypothesis、三个 operator 或 `F-C_post` headline contrast，只把原 FPCT-1 前瞻性拆分为：

| Substage | 内容 | 资源 | 当前授权 |
|---|---|---|---|
| FPCT-1A | ambiguity support audit protocol locking | R0，文档/manifest only | 当前阶段 |
...
FPCT-2 的依赖前移为 `FPCT-1B LIMITED GO 或 CROSS-PAIR GO + FPCT-1C GO + 人工授权`；`NO-GO` 或 `INCONCLUSIVE` 不满足该依赖。
...
- Pair eligibility 与 pilot selection 只使用 `fit+calibration` label-free support；
...
- `m=1`：one-to-one；
- `m>=2`：nominal ambiguous；
- gate-effective ambiguous：`m>=2` 且达到获批 secondary-mass threshold。

`A` 只取 raw alignment `source_weights`，不得乘 confidence、entropy、gate 或 selector。

只有所有 eligible parent 均为 `m<=1` 的 sample 才是 exact no-factorization control。没有 gate-effective parent 但存在 `m=2` 的 sample 属于 below-threshold nominal control；`F-C_post` 在该 stratum 不保证为 0，必须与 exact control 分开报告。

### 17.3 Threshold grid 与当前 review gate
...
- sensitivity lenient：`S>=1/2`；
- recommended gate-primary：`S>=2/3`；
- sensitivity strict：`S>=3/4`。

`S>=2/3` 是不退化为 nominal `m>=2` 的最小离散 threshold，但把 `m>=3` 作为 support-gate practical boundary 仍需人工批准；正式 matched-training structural population 继续是 `m>=2`。Support floor 推荐以 distinct content group 为 selection unit、三任务 task-macro harmonic effective size为识别量，并使用 3 pairs × 3 tasks 的 Bonferroni simultaneous Wilson LCB。`task_macro_cluster` estimand、`delta_pos`、`delta_direct_all`、alpha、power 与 familywise LCB confidence 均需人工批准。

因此 FPCT-1A 当前为 `REVIEW REQUIRED`，FPCT-1B `NOT AUTHORIZED`。

### 17.4 FPCT-1B decision boundary

任一 required pair/task/input/hash/schema 不完整或存在未解决 integrity failure 时，优先判 `INCONCLUSIVE`；只有全部 required cells 完整后才按 heterogeneous pass count 判定：

- 0 个 heterogeneous pair 通过：`NO-GO`；
- 1 个通过：`LIMITED GO`，只允许进入 single-pair pilot；未来机制 claim 仍需后续证据且 scope 限于该 pair；
- 至少 2 个通过：`CROSS-PAIR GO`；
- audit/hash/schema 不完整：`INCONCLUSIVE`。

这些结论只说明现有数据的识别能力，不说明 FPCT 数学成立与否，也不说明 task accuracy 会改善与否。

完整输入 hashes、support-floor formula、pilot tie-break、output schema 与排序合同见 `FPCT_1A_AMBIGUITY_PROTOCOL.md` 和 `recipe/eval_recipe/fpct_1a/ambiguity_protocol_manifest.json`。
