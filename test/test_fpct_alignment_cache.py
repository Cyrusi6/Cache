...
def test_frozen_cache_can_supply_the_diagnostic_prefix(tmp_path):
    item = {"input_ids": [[1], [2]], "labels": [1], "kv_cache_index": torch.tensor([[1, 0]])}
    path = tmp_path / "cache.pt"
    torch.save({"schema_version": 1, "alignment_sanitizer": "certified_slot0_v1", "top_k": 2, "items": [item, item]}, path)
    dataset = AlignedChatDataset([[]], SimpleNamespace(strategy=None), soft_alignment_top_k=2, fpct_alignment_sanitizer="certified_slot0_v1", fpct_alignment_cache_path=str(path))
    assert len(dataset.fpct_alignment_cache) == 1
