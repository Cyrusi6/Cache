...
## 2026-07-19：Phase 2A-1 sealed test 完成与 A-tier predictability NO-GO

### 研究目标

在完全冻结候选、特征、阈值、comparator、代码和模型后，只运行一次 held-out test，判断现有 A-tier pre-transfer summaries 是否足以实现 calibrated null/no-transfer；任一六项联合门槛失败即停止复杂 selector。

### 核心改动

- 完成四级提交冻结：implementation `3deca6ae7372dca5b2964b3aa647d4d8199bc3cd`、design `f7e87c3b97577dac3955cdbfaf1b39981ffa546d`、selection artifacts `7bce53ddcc91a47799ca4488b4ea5d696a744ef9`、attempt receipt `488adfedb30824fad08b2ae68636fe7eedfa9dc1`。
- 正式读取 test 前原子创建并推送远端 tag `phase2a1-selector-killtest-consumed-9fa1f0a`，其目标固定为 attempt commit；tag 已存在时脚本永久拒绝再次 evaluate。
- outcome-free preflight 重放 11 个 development scopes 的 calibration threshold、comparator、model-selection checksum 与模型状态，并确认 test correctness 访问为 0、五项特征跨 seeds 无漂移。
- sealed test 生成 142 行 aggregate、英文报告、中文摘要、result manifest 和 55MB local per-example 文件；正式结果文件为 write-once，SHA 已由 result manifest 全量核验。

### 实验配置

- Global primary candidate 由 ordinal tie-break 记为 `stump_cot_input_length`，但 12/12 候选在 calibration 上全部选择 `always_fused` threshold，model-selection accuracy/transfer 也全部相同；因此候选名不代表 input length 获得了可识别优势。
- Calibration-selected comparator 同为 `always_fused`。Global test、same-rate random、三 hetero pairs、strict cross-family 与 same-tokenizer control 均使用冻结的同一比较规则。
- Primary test 只包含 content-group test split；LOTO 按预注册在 held task 的全部四个 split 上评测，LOSO/LOPO 只评 held seed/pair 的 test groups。

### 验证结果

- Pair-balanced task-macro：receiver 39.0480%、fused/selector/comparator 49.3167%、oracle 59.0816%；selector−best-fixed `0.00 pp`，95% CI `[0.00,0.00] pp`，oracle-over-best-fixed 仍有 `+9.7650 pp`，但 recovery 为 0%。
- Sample-weighted：receiver 37.2962%、fused/selector 47.1524%、oracle 56.2840%；delta `0.00 pp`，oracle headroom `+9.1316 pp`，recovery 0%。
- Selector transfer rate 100%；全部 harmful events 9.7650% 被接受，harm reduction 0%；全部 beneficial events 20.0336% 被保留，benefit retention 100%。Same-rate random 因同样 100% transfer 与 selector 完全相同。
- Benefit AUPRC 0.19185，低于 benefit prevalence 0.20034；harm AUPRC 0.10114，仅略高于 harm prevalence 0.09765。Multiclass Brier 0.45975 也略差于 constant class-prior 的 0.45751；ECE 2.27% 只表示概率接近边际频率，不提供可用于 abstention 的排序/决策价值。
- 四个 pair、三个 seed、三个 task 的 global delta 全为 0；strict cross-family 与 same-tokenizer control 也均为 0。10 个 LOO folds 中 9 个退化为 100% transfer/0 delta；leave-one-task OpenBookQA 为 −0.15 pp，CI `[−0.70,+0.08] pp`。
- 六项 gate 仅 beneficial retention 通过；primary effect、CI、hetero sign、oracle recovery、harm reduction 五项失败。全量 CPU tests `250 passed, 2 warnings`，Phase 2A-1 定向合成 tests `11 passed`。

### 结论

现有五项 A-tier summaries 没有通过最小 predictability kill-test。失败不是 oracle opportunity 不足，而是所有 global candidates 在 calibration 上都回到 always-fused，无法预测何时应拒绝 transfer。按预注册停止复杂/神经 selector，不自动进入下一阶段；只有获得明确授权后，才可考虑一次新的 pre-transfer cache-geometry instrumentation。本结果不能支持通用 selector，也没有证据要求转向 pair-specific metadata policy。
