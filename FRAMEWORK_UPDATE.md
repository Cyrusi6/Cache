...
- 首个镜像 build 后的 contract review 发现三臂会分别重新 tokenization/alignment。该镜像未运行 GPU；新增一次性 128/2048 certified alignment cache materializer、只读 dataset cache 与三臂相同 sidecar SHA 绑定后，targeted 为 `72 passed`，完整 CPU-safe suite 为 `373 passed, 2 warnings`。
