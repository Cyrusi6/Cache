...
- Operative scientific SHA：`371e72f14da41f5509eafa21553c7a7418c9a53e`。
...
- Sealed no-model container fingerprint：`acfdea9dd6ee52040856bfdc037cd28c10b66b1036cc19fa7ee9f470e4db1800`。
...
Corrected synthetic gate 通过全部 reference/null/tolerance checks 后、仍在 pretrained output 前，resource smoke 的单次 latency 占位被识别为不符合 frozen median/p95 contract。实现改为每 arm 1 次 warmup + 7 次测量，硬检验 median ratio≤1.50、p95 ratio≤1.75，并在结构 layout warmup 后用 CUDA profiler 检查 FPCT forward 是否出现 host synchronization。该变更再次使 image/run lock 失效并要求从 GPU gate 重跑。

## 2026-07-20：FPCT frozen GPU gate execution and hard stop

### 研究目标

在最终 pre-output run lock 下执行 synthetic GPU numerical gate 和条件式、无 accuracy 的真实 TinyLlama→Qwen3 pretrained smoke；只有全部 integrity/resource controls 通过时才允许 matched smoke 和 12-seed confirmatory training。

### 核心改动

- 未修改 scientific code、threshold、seed、training recipe 或 checkpoint rule。
- 使用 run UID `fpct-cfm-371e72f1-20260720`、digest-addressed image、sealed init probe、只读模型/sidecar 和可恢复 controller。
- 新增 tracked GPU/K8s/confirmatory terminal reports 与小型 hard-stop manifest；详细 JSON、submitted YAML 和 controller ledger 保留在独立 `/netdisk` run root。

### 实验配置

- Scientific SHA：`371e72f14da41f5509eafa21553c7a7418c9a53e`。
- Run-lock SHA256：`2a4db8f26def997c95b590a34718916b772f686f5c00eabb2f2b69f0dfe5e5ec`。
- Image digest：`sha256:c851056733f3b7affc85ae5dabd870043f3ae7d3010d245705f5b9ded8dc36ab`。
- Node/device：`4090-48gx2` / NVIDIA GeForce RTX 4090。
- Synthetic activation-null floor：`0.0390625`，在 pretrained output 前冻结。

### 验证结果

- Final synthetic GPU gate=`GO`：FP32 reference max abs `2.384e-7`、gradient relative-L2 `1.580e-7`；FP16/BF16、invalid exact-zero、m≤1、replicated-collapse greedy controls 全通过。
- Pretrained smoke 的 finite、m≤1、HBM、median/p95 latency gates 通过；F/C_post median ratio `1.2769`、p95 ratio `1.2839`，F peak HBM `4.1710` GiB。
- 三个硬失败：activation output delta 为 0；replicated-collapse maximum output delta `0.71875`；profiler 检出 `cudaDeviceSynchronize` 和 `cudaStreamSynchronize`。
- Job 没有 eviction/preemption/node/storage failure，未触发 infrastructure-only retry。

### 结论

当前 frozen execution 判定 `GPU_ENGINEERING_BLOCKED` 并进入 controller terminal state。未提交 matched-training smoke 或 seeds 45–56；未训练、未创建 checkpoint、未运行 model-selection/held-out accuracy，因此没有 T/O/I/N 或性能/机制结论。任何恢复都必须作为新的 prospective revision 重新冻结 scientific code、image 和 run lock，不能覆盖或继续本次失败 execution。

## 2026-07-20：FPCT-GPU-R2 prospective root-cause lock

### 研究目标

在任何新 pretrained output 前，前瞻冻结旧 smoke 的四个候选根因、operator-isolated diagnostic matrix、canonical FP32 prior、逐层 first-divergence、metric-specific null floors 和 scope-aware profiler contract。

### 核心改动

- 冻结 H1 fresh checkpoint-native gate zero、H2 BF16 prior drift、H3 backend auto-dispatch、H4 profiler scope confound；禁止看到新输出后新增假设并原地继续正式实验。
- 将 activation 指标改为同 packed path 的 `Delta_fact=F_real-F_replicated_atoms`，并拆出 `Delta_rep`、exact collapse-to-parent `Delta_bypass` 与 m≤1 controls。
- 冻结 8 个独立进程 operator conditions；forced-on 仅为 parameter-free label-free engineering canary。
- 冻结 canonical FP32 A/logA/mask/C_post reduction/diagnostics/softmax contract及 `2e-7` row invariants。
- 冻结 18-row fit/calibration panel：3 tasks × certified m2/m3/m4 × 每 cell 2 个按 content-group SHA 排序的 groups；未读取 label/correctness。
- 冻结 9 个 metric-specific numerical-null floors、4 seeds、6 depths、4 cardinalities、99.9 percentile 与 2× safety multiplier。
- 冻结 P0–P6 profiler panels和 scientific/harness/profiler scope边界。

### 验证边界

当前只读取旧代码、config 和 sealed label-free geometry artifacts；未运行 tokenizer、自然 prompt、新 pretrained forward、GPU、训练或 accuracy。旧 `371e72f...` image/run-lock/Jobs/artifacts 保持不可变。下一步必须先提交并推送本 prospective protocol；clean containing commit 才能成为 R2 pre-output SHA。

## 2026-07-20：FPCT-GPU-R2 scientific recovery 与 CPU/HF hardening

### 研究目标

在不产生新 pretrained output 的前提下落实 R2 H1–H4 修复，建立新的 complete synthetic gate、operator-isolated natural diagnostic、scope-aware profiler 和 matched-training integrity path；旧 terminal execution 永久保持不可变。

### 核心改动

- 零输出 probe 确认 fresh projector 无 checkpoint、28 层 key/value gate logits 全为 0，旧 activation null 正式分类为 `EXPECTED_NATIVE_NULL`；旧 config 没有 explicit eager proof。
- alignment prior 在 CPU 侧 mask、FP32 normalize、log-space canonicalize 并 hash；合法行 `logsumexp(logA)` 与 `sum(exp(logA))` 均受 `2e-7` 硬门约束。
- C_post/F 共用 Qwen eager adapter；attention logits、mask、softmax、value reduction和 C_post weighted K/V collapse 使用 FP32 accumulation。
- exact collapse-to-parent bypass 与 replicated-atoms 分离；F/C_post 保持相同 candidate fuser、call order、parent nuisance 和 prior SHA，无 F-only parameter。
- layout 使用 CPU-certified max-slot/source-length metadata，GPU hot path不执行 CUDA scalar sizing、prior Python validation 或 projector host diagnostics。
- 自然诊断扩为 FP32/BF16 × 8 operator conditions 的 sealed 独立进程；local-only compact traces只保存冻结 parent 与 last-token，用于逐层 first-divergence。
- P0–P6 profiler parser按 `fpct.pack`、`fpct.attention`、`project_candidates`、scientific/harness/profiler scopes区分同步来源。
- 四步 training smoke 新增 step-0/RNG/data-order/eager、pre-collapse candidate、parent nuisance、Gumbel、candidate-sensitive gradient、mask、scheduler、checkpoint reload硬门，并确保生成官方 `checkpoint-4`。
- 新增 R2 controller、run-lock-bound K8s renderer 和独立 R2 GPU/pretrained/matched/formal templates；尚未渲染或提交任何 Job。

### 验证结果

- R2 numerics/runner/production/Qwen 定向测试通过。
- 扩展定向集 `53 passed`。
- 首次 CPU-safe full suite 使用 `/tmp` basetemp 得到 `396 passed, 2 failed, 2 warnings`；两个失败均是既有 Route1 manifest path contract 拒绝 repo 外 `/tmp`，不是 scientific regression。随后按仓库约定在 `local/tmp` 重跑为 `401 passed, 2 warnings`。
- `py_compile` 与 `git diff --check` 通过。

### 当前结论

R2 当前只到 CPU/HF engineering readiness。没有运行新 pretrained model forward、GPU/CUDA、Kubernetes、训练、checkpoint 或 accuracy。下一步必须先形成 clean pushed scientific SHA，再构建全新 immutable image/run-lock并从 complete synthetic GPU gate开始。

## 2026-07-20：FPCT-GPU-R2 immutable image 与 pre-output run lock

### 核心配置

- Scientific SHA：`9f2ffcd9ff21e4575f8fe870167eb04a7c86edb5`，local/upstream一致。
...
### R2g-v2 terminal result

- Sealed GPU gate=`GO`；16 conditions+5 profiles完整；22/23 checks通过。
